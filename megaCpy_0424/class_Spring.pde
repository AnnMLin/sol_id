class Spring {
  ...
  void showMe() {
    ...
    else if (PVector.dist(p00.pos, p01.pos) > gridInc && PVector.dist(p00.pos, p01.pos) <= gridInc*1.1){
      stroke(18, 100, 100);
    }
    else if (PVector.dist(p00.pos, p01.pos) > gridInc*1.1 && PVector.dist(p00.pos, p01.pos) <= gridInc*2){
      stroke(0, 100, 100);
    }
    strokeWeight(0.5);
    noFill();
